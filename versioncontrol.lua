...
CreateThread(function()
    ...
    PerformHttpRequest(string.format('https://api.github.com/repos/%s/%s/releases/latest', github, repo), function(status, res)
        if status ~= 200 or not res then return Warning('Unable to check for updates') end
        res = json.decode(res)

        if res.draft or res.prerelease or not res.tag_name then return end

        if isVersionOlder(cVer, res.tag_name) then
            Warning(string.format("You're running an outdated version of %s (current version: %s)!", resource, cVer))
            Warning(string.format("Download the latest version (%s) here: %s", res.tag_name, res.html_url))
            return
        else
            Success(string.format('%s is up to date!', resource))
            return
        end
    end, 'GET')
end)